      ...
      <section className="mb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="rounded-xl overflow-hidden bg-[var(--card)] border border-[var(--border)] animate-pulse aspect-[3/4]" />
          ))}
        </div>
      </section>
    );
    ...
    <section className="mb-16">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        {galleries.slice(0, 4).map((gallery) => (
          <div
            key={gallery.id}
            className="group relative rounded-xl overflow-hidden cursor-pointer aspect-[3/4] bg-[#18181B] shadow-[0_5px_15px_rgba(0,0,0,0.5)] border border-[var(--border)] hover:border-[var(--gold)]/40 transition-all hover:-translate-y-1"
