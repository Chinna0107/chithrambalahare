...
const Reviews = () => {
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Are you sure you want to delete ${selectedIds.length} reviews?`)) return;
    setIsSaving(true);
    try {
      await Promise.all(selectedIds.map(id => axios.delete(`/api/reviews/${id}`)));
      await refreshDb();
      window.dispatchEvent(new Event('tolly_db_change'));
      triggerNotification(`${selectedIds.length} reviews deleted!`);
      setSelectedIds([]);
    } catch {
      triggerNotification('Failed to delete some reviews.', 'error');
    } finally {
      setIsSaving(false);
    }
  };
